    ...
    {\small
        \begin{itemize}
            \item[]<1-> \begin{block}{Basic Idea} Solve a fully non-linear, low-order (LO) system
                that preserves a high-order (HO) solution from efficent MC
                simulations, \emph{for each time step}\end{block}
                \vspace{-0.321in}
              \item<2-> The LO system consists of space-angle moment equations, formed over a fixed finite-element (FE) spatial mesh
                \begin{itemize}
            ...
            \item<3-> The HO system is a fixed-source, pure-absorber transport problem
                \begin{itemize}
                    ...
                    \item<3-> Solved with ECMC for efficient reduction of statisical noise 
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            &= \frac{I^{n}}{c\Delta t} + \boxed{\frac{1}{2}\sigma_a a c T_{LO}^{n+1,4}} \\
     ...
     \end{align*}
        ...
        \item Compute $\tilde{\epsilon}^{(m)} = \B L^{-1} \tilde{r}^{(m)}$ with MC 
            \begin{itemize}
                ...
                \item Use cell-wise \colb{systematic} sampling for $r^{(m)}$ source
            \end{itemize}
            \pause
...
\begin{frame}
 ...
 \item \u{For all results}
    ...
    \item Fixed $\Delta t$ of $0.001$ sh
    \item One HO solve per time step
    \end{itemize}
    \end{itemize}
\end{frame}
