...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Linear discontinuous (LD) FE in space for $\phi$ and $T$, and half-range
            averages in angle
    \end{itemize}
    \begin{centering}
    \begin{tikzpicture}
...
\begin{frame}
    ...
    \item We take spatial and angular moments of the equations to reduce
        dimensionality.
    \item Use a backward Euler discretization in time
...
\begin{frame}
    ...
    \begin{itemize}
        \item The streaming operator $D$ containts unknown weighted angular averages,
            called consistency terms
...
\begin{frame}
    ...
    \begin{minipage}[t]{0.70\linewidth}
        \vspace{-1.6in}
        \begin{itemize}
            \item {\small $\displaystyle \tilde I(x,\mu) = I_{a,i} + I_{x,i}
                \frac{2}{h_{x}}(x-x_i) + I_{\mu,i}
            \frac{2}{h_{\mu}}(\mu-\mu_i)$}
        \item \colb{Path-length} estimators of moments, e.g.
            {\small
            \begin{align*}
                I^{n+1}_{x,i} &= \frac{6}{h_{x}^2h_\mu}
             \iint\limits_\mathcal{D} (x_{c,j}-x_i) I^{n+1}(x,mu) \d x \d \mu 
        \end{align*}}    
        \end{itemize}
    \end{minipage}
    \begin{itemize}
        \item Discrete in time, resulting in fixed-source problem
...
\begin{frame}
    ...
    \begin{block}{}
    \begin{itemize}
        \item Comparse statistical accuracy of HO solvers
        \item One HO solve, with a \emph{fixed number of histories} per time step. Comparison of
